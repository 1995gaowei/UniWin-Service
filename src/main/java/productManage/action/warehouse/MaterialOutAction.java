...
//  创建出库单时传递的数据：
	...
	public String showAllStores(){
		...
		List<Store> results = warehouseService.getAllStore();
		jsonMap = new HashMap<>();
...
//    	}
    ...
    public List<StoreAndApply> storeApplyConversion(List<Object[]> queryResults){
    	...
    	for(Object[] os : queryResults){
        	...
        	sa.setMaterialApplyVol( ((Float)os[1]).floatValue());
    ...
    public String showMaterialOutputs(){
		List<MaterialOutput> list = warehouseService.getMaterialOutputList();
		jsonMap = new HashMap<>();
		jsonMap.put("result", "success");
